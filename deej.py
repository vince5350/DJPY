...
class Deej(object):
    ...
    def start(self):
        ...
        while not self._stopped:
            ...
            clean_values = [self._clean_session_volume(n) for n in settings.sliders]
    ...
    def _refresh_sessions(self):
        ...
        # active_speaker_device = utils.AudioUtilities.GetSpeakers()
        # active_device_interface = active_speaker_device.Activate(IAudioEndpointVolume._iid_, CLSCTX_ALL, None)
        # self._master_session = cast(active_device_interface, POINTER(IAudioEndpointVolume))
        self._master_session = AudioUtilities.GetSpeakers()

        # take the microphone session
        # active_mic_device = AudioUtilities.GetMicrophone()
        # active_device_interface = active_mic_device.Activate(IAudioEndpointVolume._iid_, CLSCTX_ALL, None)
        # self._mic_session = cast(active_device_interface, POINTER(IAudioEndpointVolume))
        self._mic_session = AudioUtilities.GetMicrophone()
    ...
    def _apply_volume_changes(self):
        for targets in iter(self._settings['slider_mapping']):

            slider_value = self._slider_values
            target_found = False
    ...
    def _acquire_target_sessions(self, name):
        ...
        if name == 'mic':
            return [('Microphone', self._mic_session)]

        for process_name, process_sessions in self._sessions.items():
            if str(process_name).lower() == str(name).lower():
...
def setup_tray(edit_config_callback, refresh_sessions_callback, stop_callback):
    ...
    tray = infi.systray.SysTrayIcon('assets/logo.ico', 'DJPY', menu_options, on_quit=lambda _: stop_callback())
    tray.start()
...
async def main():
    ...
    try:
        ...
        with open(filename, 'w') as f:
            ...
            f.write('If you\'ve just encountered this, please contact @vince5350 and attach this error log.\n')
